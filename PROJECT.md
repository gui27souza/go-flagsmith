...
- [X]  **Integração com SDK:** Configurar cliente Flagsmith com cache em memória e intervalo de sincronização.
- [X]  **Padrão Fallback Seguro:** Garantir que falhas de rede ou timeout na API do Flagsmith não causem HTTP 500 no serviço (uso de valores default predefinidos).
...
- [X]  **Observabilidade & Health Check:** Endpoints `/healthz` (liveness) e `/readyz` (readiness) refletindo a integridade da aplicação.
